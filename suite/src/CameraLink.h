#pragma once
// CameraLink.h — suite ⇄ capture agent client for the CAMERA bus.
//
...
// XylodLink. The camera lives on the capture PC (where the suite runs), so the
// default host is loopback; override with CAMERA_HOST.
//
// Read-only in the suite: it mirrors live camera state (line rate, TDI stages,
// gain, scan direction, model, link mode) for the status panel. Setting those
// stays on the HMI, so there is no setParam here.
...
class CameraLink : public QObject
{
    ...
    // mirrored camera state (pushed on connect and after every change on the bus)
...
private:
    ...
    QString m_host = QStringLiteral("127.0.0.1");   // capture PC (local); CAMERA_HOST overrides
    int     m_port = 5521;
